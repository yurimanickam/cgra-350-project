...
void LavaLamp::updateBlobPhysics(LavaBlob& blob, float dt) {
    ...
    acceleration -= blob.velocity * m_viscosity; // viscous drag
    ...
    for (size_t i = 0; i < originalSize; ++i) {
        if (m_blobs[i].radius > maxRadius) {
            ...
            glm::vec3 offset(
                ...
                glm::abs(m_randomDist(m_rng)) * 0.4f,
                m_randomDist(m_rng) * 0.4f
            );
            ...
            newBlob.velocity += offset * 0.15f;

            m_blobs[i].radius = newRadius;
            m_blobs[i].position -= offset;
            m_blobs[i].velocity -= offset * 0.15f;
...
void LavaLamp::removeBlob() {
    if (!m_blobs.empty()) m_blobs.pop_back();
}
...
std::vector<glm::vec4> LavaLamp::getBlobPositions() const {
    ...
    for (const auto& b : m_blobs) out.push_back(glm::vec4(b.position, 1.0f));
    // Ensure at least one element to avoid UB when passing pointer to glUniform (caller clamps count)
    if (out.empty()) out.push_back(glm::vec4(0.0f));
    return out;
}
...
std::vector<float> LavaLamp::getBlobRadii() const {
    ...
    out.reserve(m_blobs.size());
    for (const auto& b : m_blobs) out.push_back(b.radius);
...
std::vector<float> LavaLamp::getBlobBlobbiness() const {
    std::vector<float> out;
    out.reserve(m_blobs.size());
    for (const auto& b : m_blobs) out.push_back(b.blobbiness);
    if (out.empty()) out.push_back(0.0f);
    return out;
}
...
std::vector<glm::vec3> LavaLamp::getBlobColors() const {
    ...
    out.reserve(m_blobs.size());
    for (const auto& b : m_blobs) out.push_back(b.color);
    if (out.empty()) out.push_back(glm::vec3(1.0f, 0.3f, 0.0f));
    return out;
}
