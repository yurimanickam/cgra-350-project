...
vec2 rayCylinderIntersect(vec3 ro, vec3 rd, float radius, float minY, float maxY) {
    ...
    if (abs(a) < 1e-8) {
        ...
        vec3 p = ro + rd * tCap;
        if (tCap >= 0.0 && (p.x * p.x + p.z * p.z) <= radius * radius) return vec2(tCap, tCap);
        tCap = (maxY - ro.y) / rd.y;
        p = ro + rd * tCap;
        if (tCap >= 0.0 && (p.x * p.x + p.z * p.z) <= radius * radius) return vec2(tCap, tCap);
        return vec2(-1.0);
    ...
    if ((y0 < minY && y1 < minY) || (y0 > maxY && y1 > maxY)) {
        if (abs(rd.y) > 1e-8) {
            ...
            vec3 p = ro + rd * tPlane;
            if (tPlane >= 0.0 && (p.x * p.x + p.z * p.z) <= radius * radius) return vec2(tPlane, tPlane);
...
void main() {
    ...
    if (uRenderMode == 0) {
        ...
        FragColor = vec4(shaded, 0.35); // slightly more visible glass
        return;
    }
